...
class Install :
	...
	def adb_reboot_system(self) :
		while self.running :
			if self.reboot_system :
				reboot = os.system("cd platform-tools & adb reboot")
				self.reboot_system = False
				self.running = False
	...
	def draw_windows(self) :
		...
		while self.running :			
			...
			if self.rebooting :
				...
				self.screen.blit(self.loader_anim, (885, 700))
				text = small_font.render("Rebooting into recovery", 1, WHITE)
				self.screen.blit(text, (800, 800))
			...
			if self.reboot_system :
				...
				self.screen.blit(pygame.transform.scale(render, (500, 350)), (700, 350))
				self.screen.blit(self.loader_anim, (885, 700))
				text = small_font.render("Rebooting into system", 1, WHITE)
				self.screen.blit(text, (800, 800))
	...
	def start_installation(self, file) :
		...
		thread_7 = threading.Thread(target = self.adb_reboot_system)
